...
import pickle
import os,sys
def run(is_training=True, render=False, readq=False):

    #establish new directory for saving results
    if not os.path.exists('runs'):
        os.makedirs('runs')
    workbase = np.datetime64('now', 'run_YYYY_MMDD_HHMMSS').astype(str)
    os.makedirs(os.path.join('runs', workdir))
    workdir = os.path.join('runs', workdir) 
    os.chdir(workdir)
    ...
    divisions = 5               # used to convert continuous state space to discrete space
    ...
    while(True):
        ...
        while(steps < 1000 or is_training==False):
            ...
            s_i1 = ns_i1
            s_i2 = ns_i2
            print(f"type reward = {type(reward)} = {reward}")
...
if __name__ == '__main__':
    run(is_training=("train" in sys.argv[1:]),
        render=("render" in sys.argv[1:]),
        readq=("readq" in sys.argv[1:])
    )
